...
<script>
  try {
    var sentient = {
      text: function(text, tone, intensity) {
        var lvl = intensity || 0.5;

        // Phase 1: Strip AI-telltale phrases
        var aiPhrases = [
        ...
        if (tone === 'casual') {
          text = text.replace(/\\b(cannot|do not|will not|is not|are not|would not|should not|could not|has not|have not|did not)\\b/gi, function(m) {
            var map = {"cannot":"can't","do not":"don't","will not":"won't","is not":"isn't","are not":"aren't","would not":"wouldn't","should not":"shouldn't","could not":"couldn't","has not":"hasn't","have not":"haven't","did not":"didn't"};
            return map[m.toLowerCase()] || m;
          });
          var starters = ["Basically, ","So, ","Honestly, ","Look, ","Here's the deal: ","Real talk, ","Okay so, ","Alright, ","Thing is, ","Not gonna lie, "];
          if (Math.random() < lvl*1.3) text = starters[Math.floor(Math.random()*starters.length)] + text.charAt(0).toLowerCase() + text.slice(1);
          var connectors = [" And "," Plus "," Also "];
          text = text.replace(/\\. ([A-Z])/g, function(m, c) { return Math.random() > (1-lvl*0.8) ? "."+connectors[Math.floor(Math.random()*connectors.length)]+c.toLowerCase() : m; });
          if (lvl > 0.6) {
            var fillers = [" honestly","  — you know?"," tbh"," basically"," right?"];
            var sArr = text.split('. ');
            text = sArr.map(function(s, i) { return i > 0 && Math.random() > 0.7 ? s + fillers[Math.floor(Math.random()*fillers.length)] : s; }).join('. ');
          }
        } else if (tone === 'professional') {
          var proSwaps = {"very":"quite","big":"substantial","small":"modest","good":"strong","bad":"suboptimal","thing":"element","stuff":"material","got":"obtained","a lot of":"considerable"};
          Object.keys(proSwaps).forEach(function(k) { if(Math.random()<lvl) text = text.replace(new RegExp('\\\\b'+k+'\\\\b','gi'), proSwaps[k]); });
        } else if (tone === 'story') {
          var openers = ["Picture this — ","Imagine this: ","So here's what happened — ","Let me set the scene — ","Here's the thing — "];
          text = openers[Math.floor(Math.random()*openers.length)] + text.charAt(0).toLowerCase() + text.slice(1);
          var transitions = [". Then ",". After that, ",". Next thing you know, ",". And — get this — "];
          text = text.replace(/\\. ([A-Z])/g, function(m, c) { return Math.random() < lvl*0.7 ? transitions[Math.floor(Math.random()*transitions.length)]+c.toLowerCase() : m; });
        } else if (tone === 'academic') {
          var acSwaps = {"show":"illustrate","think":"posit","use":"employ","help":"facilitate","start":"initiate","part":"component","check":"evaluate"};
          Object.keys(acSwaps).forEach(function(k) { if(Math.random()<lvl) text = text.replace(new RegExp('\\\\b'+k+'\\\\b','gi'), acSwaps[k]); });
          if (Math.random() < lvl) text = text.replace(/\\. ([A-Z])/g, function(m, c) { return Math.random()>0.6 ? ". As such, "+c.toLowerCase() : m; });
        }
        return text.trim();
      },
      code: function(code, style, intensity) {
        var lvl = intensity || 0.5;
        if (style === 'debug') {
          var logs = ['console.log("DEBUG:",{data})','console.trace("stack")','console.time("perf")','debugger;','console.warn("check this")','console.log(">>> here <<<")','console.table({data})'];
          return code.replace(/{/g, function(m) { return Math.random() < lvl ? '{\\n  ' + logs[Math.floor(Math.random()*logs.length)] + ';' : m; });
        } else if (style === 'commented') {
          var comments = ["// TODO: needs review","// FIXME: edge case","// NOTE: temp solution","// HACK: works for now","// OPTIMIZE: refactor later","// XXX: revisit this","// ?: why does this work","// CHANGED: was different before"];
          return code.split('\\n').map(function(line) { return Math.random() < lvl*0.75 && line.trim().length > 3 ? line + '  ' + comments[Math.floor(Math.random()*comments.length)] : line; }).join('\\n');
        } else if (style === 'minified') {
          return code.replace(/\\/\\/.*$/gm,'').replace(/\\/\\*[\\s\\S]*?\\*\\//g,'').split('\\n').map(function(l){return l.trim();}).filter(function(l){return l.length>0;}).join(lvl>0.7?'':' ');
        } else {
          code = code.replace(/const /g, function() { return Math.random() < lvl*0.5 ? "let " : "const "; });
          var names = {"result":"res","index":"idx","element":"el","value":"val","temporary":"tmp","error":"err","response":"resp","callback":"cb","function":"fn","parameter":"param"};
          if (lvl > 0.4) Object.keys(names).forEach(function(k) { if(Math.random()<lvl*0.4) code = code.replace(new RegExp('\\\\b'+k+'\\\\b','g'), names[k]); });
    ...
    var history = {
      items: JSON.parse(localStorage.getItem('sentient_history')||'[]'),
      save: function(entry) { this.items.unshift(entry); if(this.items.length>20)this.items.pop(); localStorage.setItem('sentient_history',JSON.stringify(this.items)); },
      clear: function() { this.items=[]; localStorage.removeItem('sentient_history'); }
    };
    ...
    document.addEventListener('DOMContentLoaded', () => {
      ...
      const renderOptions = (m) => {
        if (m === 'text') {
          optionsContainer.innerHTML = \`
            <select id="tone-select" class="option-select">
              <option value="casual">Casual</option>
              <option value="professional">Professional</option>
              <option value="story"Selected>Storytelling</option>
              <option value="academic">Academic</option>
            </select>
            <div class="strong-mode-toggle">
              <input type="checkbox" id="strong-mode" class="strong-checkbox">
              <label for="strong-mode">Strong (Free Cloud)</label>
            </div>
          \`;
        } else {
      ...
      const doConvert = async () => {
        ...
        const isStrong = document.getElementById('strong-mode')?.checked;
        ...
        if (mode === 'text' && isStrong) {
          try {
            const response = await fetch('/api/humanize', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({ text: input.value, tone: option })
            });
            const data = await response.json();
            if (data.error) throw new Error(data.error);
            
            const result = data.text;
            history.save({mode: 'strong', option, intensity: lvl, input: input.value, output: result, time: Date.now()});
            renderHistory();
            output.value = result;
          } catch (err) {
            console.error(err);
            alert("Error: " + err.message + ". Make sure you set GROQ_API_KEY in .env");
          } finally {
            btn.classList.remove('processing'); btn.innerHTML = '<i data-lucide="zap" class="icon-sm"></i> Humanize'; lucide.createIcons(); statusDot.className='status-dot online'; statusText.textContent='Ready'; scanLine.classList.remove('active'); updateStats(input.value,output.value);
          }
        } else {
          setTimeout(() => {
      ...
      uploadBtn.className = "action-btn btn-secondary";
      uploadBtn.id = "btn-upload";
      uploadBtn.innerHTML = '<i data-lucide="upload" class="icon-sm"></i> Upload';
      document.querySelector('.actions-bar').prepend(uploadBtn);
...
:root {
  ...
  --border: rgba(99, 102, 241, 0.3); --border-hover: rgba(99, 102, 241, 0.5);
...
.strong-mode-toggle { display: flex; align-items: center; gap: 8px; font-size: 0.8rem; color: var(--accent-cyan); font-weight: 600; margin-left: 10px; }
.strong-checkbox { width: 16px; height: 16px; cursor: pointer; accent-color: var(--accent-cyan); }
...
.brand-name { font-size: 1.6rem; font-weight: 800; background: linear-gradient(135deg, #c7d2fe, #818cf8, #38bdf8); -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent; letter-spacing: -0.5px; }
...
.card-title span { background: linear-gradient(135deg, var(--accent), var(--accent-cyan)); -webkit-background-clip: text; background-clip: text; -webkit-text-fill-color: transparent; }
.card-subtitle { color: var(--text-secondary); font-size: 0.9rem; margin-top: 5px; font-weight: 400; }

/* Controls */
.controls { padding: 1rem 2.5rem; border-bottom: 1px solid var(--border); display: flex; justify-content: space-between; align-items: center; gap: 1rem; background: rgba(0,0,0,0.15); animation: fadeInUp 0.8s 0.2s both; flex-wrap: wrap; }
.mode-tabs { display: flex; background: rgba(0,0,0,0.3); border-radius: 12px; padding: 4px; border: 1px solid rgba(255,255,255,0.15); }
...
.option-select { padding: 10px 18px; border-radius: 10px; border: 1.5px solid var(--border); background: rgba(0,0,0,0.35); color: var(--text-primary); font-family: var(--font-sans); font-size: 0.85rem; font-weight: 500; cursor: pointer; outline: none; transition: border-color 0.2s; -webkit-appearance: none; appearance: none; }
.option-select:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(99,102,241,0.15); }
...
.text-input { width: 100%; min-height: 360px; padding: 1.25rem; border-radius: 16px; border: 1.5px solid var(--border); background: rgba(0,0,0,0.25); color: var(--text-primary); font-family: var(--font-mono); font-size: 0.95rem; line-height: 1.75; resize: none; outline: none; transition: all 0.3s ease; }
.text-input:focus { border-color: var(--accent); box-shadow: 0 0 0 3px rgba(99,102,241,0.15), inset 0 0 40px rgba(99,102,241,0.03); }
...
/* Actions */
.actions-bar { padding: 1.25rem 2.5rem; border-top: 1px solid var(--border); display: flex; justify-content: center; gap: 0.75rem; background: rgba(0,0,0,0.1); animation: fadeInUp 0.8s 0.4s both; flex-wrap: wrap; }
...
.btn-secondary { background: rgba(255,255,255,0.04); color: var(--text-secondary); border: 1.5px solid var(--border); }
.btn-secondary:hover { background: rgba(255,255,255,0.08); color: var(--text-primary); border-color: rgba(99, 102, 241, 0.5); transform: translateY(-2px); box-shadow: 0 4px 12px rgba(99,102,241,0.15); }
...
/* Footer */
.footer { margin-top: 1.5rem; text-align: center; color: var(--text-muted); font-size: 0.75rem; animation: fadeInUp 0.8s 0.6s both; display: flex; align-items: center; justify-content: center; gap: 6px; flex-wrap: wrap; }
...
.intensity-tabs { display: flex; background: rgba(0,0,0,0.3); border-radius: 10px; padding: 3px; border: 1px solid rgba(255,255,255,0.15); }
.intensity-tab { padding: 7px 16px; border-radius: 8px; border: 1px solid transparent; background: transparent; color: var(--text-secondary); cursor: pointer; font-weight: 600; font-size: 0.78rem; font-family: var(--font-sans); transition: all 0.25s ease; display: flex; align-items: center; gap: 6px; }
...
@media (max-width: 900px) {
  .app { padding: 1.25rem 1rem 1rem; }
  .card-header { padding: 1.5rem 1.5rem 1rem; }
  .controls { padding: 0.8rem 1.5rem; }
  .work-panel { padding: 1.25rem 1.5rem 1.5rem; }
  .actions-bar { padding: 1rem 1.5rem; }
  .stats-bar { padding: 0.5rem 1.5rem; }
  .card-title { font-size: 1.6rem; }
  .text-input { min-height: 280px; font-size: 0.88rem; }
}

/* ===== RESPONSIVE: Mobile ===== */
@media (max-width: 640px) {
  .app { padding: 0.75rem 0.5rem 0.75rem; }
  .top-bar { flex-direction: column; gap: 0.75rem; align-items: stretch; padding: 0.5rem 0.25rem; margin-bottom: 1rem; }
  .brand { justify-content: center; }
  .brand-icon { width: 38px; height: 38px; border-radius: 10px; }
  .brand-icon svg { width: 20px; height: 20px; }
  .brand-name { font-size: 1.3rem; }
  .status-bar { justify-content: center; font-size: 0.72rem; padding: 6px 12px; }

  .engine-card { border-radius: 16px; }
  .engine-card::before { border-radius: 17px; }

  .card-header { padding: 1.25rem 1rem 0.75rem; flex-direction: column; align-items: flex-start; gap: 4px; }
  .card-title { font-size: 1.35rem; gap: 6px; }
  .card-subtitle { font-size: 0.78rem; }

  .controls { padding: 0.75rem 1rem; flex-direction: column; align-items: stretch; gap: 0.75rem; }
  .mode-tabs { width: 100%; justify-content: center; }
  .mode-tab { padding: 8px 16px; font-size: 0.82rem; flex: 1; justify-content: center; }
  .option-select { width: 100%; padding: 9px 14px; font-size: 0.82rem; }
  .intensity-group { width: 100%; justify-content: center; flex-wrap: wrap; }
  .intensity-tabs { width: 100%; }
  .intensity-tab { flex: 1; justify-content: center; padding: 7px 10px; font-size: 0.72rem; }
  ...
  .text-input { min-height: 180px; padding: 1rem; font-size: 0.85rem; border-radius: 12px; }
  .io-label { font-size: 0.68rem; letter-spacing: 1.5px; margin-bottom: 0.6rem; }

  .stats-bar { padding: 0.5rem 1rem; flex-direction: column; gap: 4px; text-align: center; font-size: 0.65rem; }

  .actions-bar { padding: 0.85rem 1rem; gap: 0.5rem; flex-wrap: wrap; }
  .action-btn { padding: 10px 18px; font-size: 0.82rem; gap: 6px; flex: 1; min-width: 100px; justify-content: center; border-radius: 10px; }

  .footer { flex-direction: column; gap: 4px; font-size: 0.68rem; margin-top: 1rem; }
  .footer .shortcuts { display: none; }

  .history-panel { width: 100%; right: -100%; }
  .history-header { padding: 1rem; }
...
@media (max-width: 380px) {
  ...
  .text-input { min-height: 150px; font-size: 0.8rem; }
}
`;
  ...
  try {
    const server = Bun.serve({
      ...
      async fetch(req) {
        ...
        // API Endpoint for Strong Mode
        if (path === "/api/humanize" && req.method === "POST") {
          try {
            const { text, tone } = await req.json();
            const chunks = chunkText(text);
            let resultText = "";
            for (const chunk of chunks) {
              const res = await humanizePipeline(chunk, tone);
              resultText += res.text + " ";
            }
